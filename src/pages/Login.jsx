...
const Login = ({ setRole }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setTimeout(() => {
        navigate(user.role === "admin" ? "/dashboard" : "/guestDashboard");
      }, 800);
    } catch (err) {
      setError(err.response?.data?.message || "Login failed");
    } finally {
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div className="relative w-full">
            <input
              ...
              value={form.password}
              onChange={handleChange}
              className="w-full border border-gray-300 rounded-lg p-3 pr-10 focus:ring-2 focus:ring-blue-500"
              required
            />
            ...
            <button
              type="button"
              onClick={() => setShowPassword((prev) => !prev)}
              className="absolute inset-y-0 right-3 flex items-center text-gray-600 hover:text-blue-600 cursor-pointer"
            >
              {showPassword ? <FaEyeSlash /> : <FaEye />}
            </button>
          </div>
          ...
          <button
            type="submit"
            disabled={loading}
            className={`w-full flex items-center justify-center bg-blue-600 hover:bg-blue-700 text-white py-3 rounded-lg font-semibold shadow-md transition 
